...
// 0xFFFF); a successful OCInvoke narrows the live compartment to the
...
module tb;
  ...
  initial begin
    ...
    $display("pcc_length before OCInvoke : x9=0x%0h (must be 0xFFFF, unbounded)", dut.CPU_Xreg_val_a0[9]);
    ...
    $display("pcc_length after return    : x12=0x%0h (must be 0xFFFF again)", dut.CPU_Xreg_val_a0[12]);

    if (dut.CPU_Xreg_val_a0[9]  == 64'hFFFF &&
        dut.CPU_Xreg_val_a0[11] == 64'h0100 &&
        dut.CPU_Xreg_val_a0[12] == 64'hFFFF) begin
