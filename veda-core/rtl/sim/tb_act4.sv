`timescale 1ns/1ps
// Copied verbatim from rtl/sim/tb_act4.sv -- no adaptation needed.
// veda_core.tlv was seeded from rv64i_core.tlv and keeps the identical
// top-level module interface (`top dut(.clk, .reset, .cyc_cnt, .passed,
// .failed)`) and the identical `elfmem[]` array (same ELFMEM_BASE/
// ELFMEM_SIZE, confirmed by reading both files' own localparam
// declarations before copying this), so this testbench's own
// hierarchical `dut.elfmem[...]` references resolve identically against
// either core. Real purpose: the base RV64I ACT4 conformance suite
// (51/51) has only ever been run against the separate, untouched
// rv64i_core.tlv -- every one of Veda-Core's own fourteen RTL milestones
// checked base-ISA regression only via the much smaller, hand-assembled
// 81-instruction smoke test, never this full corpus, against the actual
// file (veda_core.tlv) that carries all the Veda-Core additions. This
// closes that real, previously-unchecked verification gap.
//
