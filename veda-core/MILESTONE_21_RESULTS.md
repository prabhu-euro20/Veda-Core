...
**The restore-and-resume-inside-compartment pattern** — CLOSED, 2026-08-10, see `MILESTONE_21_PCC_AUTO_RESTORE_RESULTS.md`. `mret`/`sret` now automatically restore the saved bounds (matching real CHERI-RISC-V's own `mepcc`-unseal-on-`mret` precedent) whenever `veda_pcc_save_and_reset()` actually saved something, self-consuming so a saved value is never applied twice; software retains full control via an explicit clear of `veda_mepcc_length` before its own `mret` if it wants a different outcome (three pre-existing tests needed exactly this migration once the new default shipped).
