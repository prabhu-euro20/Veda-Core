...
**Interrupt-path coverage** — `handle_interrupt` (`sys/sys_control.sail`) routes through the identical `trap_handler()` body this fix hooks into, and is therefore covered by the same real mechanism, but this was **not verified by a real test** in this project's own test config — no real interrupt source is currently active (no CLINT/timer wiring exists anywhere in this project's own RTL or Sail work, per this project's own prior research). Named honestly as unverified, not silently assumed safe.
